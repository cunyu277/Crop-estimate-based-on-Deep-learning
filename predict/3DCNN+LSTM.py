...
class Conv3DLSTMModel(nn.Module):
    def __init__(self, input_shape=(16, 16, 32)):  # 默认形状：(时间步, 波段数, bins)
        super().__init__()
    ...
    def forward(self, x):
        # 输入形状: (batch, time_steps, bands, bins)
        x = x.unsqueeze(1)  # 添加通道维度 (batch, 1, time_steps, bands, bins)
        
        # 3D CNN处理
        cnn_out = self.cnn(x)  # (batch, C, T, B, bins)
        batch_size = cnn_out.size(0)
        
        # 调整维度用于LSTM
        cnn_out = cnn_out.permute(0, 2, 1, 3, 4)  # (batch, T, C, B, bins)
        cnn_out = cnn_out.reshape(batch_size, cnn_out.size(1), -1)  # (batch, T, C*B*bins)
        
        # LSTM处理
        lstm_out, _ = self.lstm(cnn_out)
        
        # 回归预测
        outputs = self.regressor(lstm_out[:, -1])
...
if __name__ == "__main__":
    ...
    # 保存结果到Excel
    with pd.ExcelWriter(f"{TEST_YEAR}年产量预测评估_3DCNN_LSTM.xlsx") as writer:
