...
class SimMTMMask:
    ...
    def __call__(self, x):
        ...
        """
        # 步骤1：生成基础掩码模板
        base_mask = self._adaptive_masking(x[..., 0])  # 取第一个波段计算
        
        # 步骤2：跨波段协同（随机选择部分波段应用）
        band_mask = np.random.rand(self.bands) < 0.8  # 70%波段应用相同掩码
...
class CropNoiseAugment:
    ...
    def __call__(self, x):
        ...
        if self.simmtm_mask and np.random.rand() < 0.9:
            # 初始化（参数需根据实际数据调整）
            masker = SimMTMMask(
                base_ratio=0.15,  # 基础掩码比例
                dynamic_range=(0.2, 0.5),  # 动态调整范围
                time_steps=16,
                bands=16,
                min_scale=3,
                max_scale=6
...
def optimize_hyperparameters(train_loader, val_loader, n_trials=30):
    def objective(trial):
        # 1. 定义搜索空间
        params = {
            'cnn_channels': trial.suggest_categorical('cnn_channels', [64, 96, 128]),
            'lstm_head_size': trial.suggest_int('lstm_head_size', 16, 128),
            'lstm_num_heads': trial.suggest_int('lstm_num_heads', 2, 8),
            'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True),
            'weight_decay': trial.suggest_float('weight_decay', 1e-6, 1e-3, log=True),
            'dropout': trial.suggest_float('dropout', 0.1, 0.5)
        }
        
        # 修改模型构建方式：动态创建整个CNN模块
        class DynamicCNN(nn.Module):
            def __init__(self, channels):
                super().__init__()
                self.seq = nn.Sequential(
                    nn.Conv3d(1, channels, (3,3,3)),
                    nn.BatchNorm3d(channels),  # 通道数与卷积输出一致
                    nn.GELU(),
                    nn.MaxPool3d(2),
                    nn.Dropout3d(params['dropout']),
                    # 后续层根据需要进行类似调整
                    nn.Conv3d(channels, 128, (3,3,3), padding=1),
                    nn.BatchNorm3d(128),
                    nn.GELU(),
                    SpatioTemporalAttention3D(128),
                    nn.AdaptiveAvgPool3d((4,4,4))
                )
        ...
        # 构建完整新模型（关键修改）
        model = Conv3DxLSTMModel(input_shape=(TIME_STEPS, BANDS, BINS))
        model.cnn = DynamicCNN(params['cnn_channels'])  # 替换整个CNN模块
        model = model.to(device)
        
        # 重新计算LSTM输入维度
        with torch.no_grad():
            dummy = torch.randn(1, 1, TIME_STEPS, BANDS, BINS).to(device)
            cnn_out = model.cnn(dummy)
            lstm_input_size = cnn_out.size(1) * cnn_out.size(3) * cnn_out.size(4)
        
        # 更新xLSTM输入维度
        model.xlstm = xLSTM(
            input_size=lstm_input_size,
            head_size=params['lstm_head_size'],
        ...
        # 5. 快速验证（每个trial只训练5个epoch）
        model.train()
        for epoch in range(50):
            for X, y in train_loader:
                optimizer.zero_grad()
                pred = model(X.to(device))
                loss = nn.HuberLoss()(pred, y.to(device))
                loss.backward()
                optimizer.step()
        ...
        with torch.no_grad():
            for X, y in val_loader:
                pred = model(X.to(device)).cpu()
                val_loss += nn.HuberLoss()(pred, y).item()
...
if __name__ == "__main__":
    ...
    # 执行超参数优化（建议先小规模测试）
    best_params = optimize_hyperparameters(train_loader, val_loader, n_trials=100)
