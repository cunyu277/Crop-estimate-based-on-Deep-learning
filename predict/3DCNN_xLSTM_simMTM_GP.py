...
class SpatialGP(ApproximateGP):
    def __init__(self, num_regions, inducing_points=64):
        # --- 必须先调用父类初始化 ---
        # Step 1: 初始化变分策略
        inducing_points_tensor = torch.randn(inducing_points, 2)  # 注意维度调整为(数量, 2)
        ...
        # Step 2: 初始化嵌入层（必须在父类初始化后）
        self.embedding = nn.Embedding(num_regions, 2)  # 位置正确
        
        # --- 剩余组件初始化 ---
        self.mean_module = LinearMean(input_size=2)
        self.covar_module = ScaleKernel(
            RBFKernel(ard_num_dims=2, requires_grad=True), 
            ard_num_dims=2
        )

    def forward(self, region_ids):
        # 确保region_ids是1D张量
        region_ids = region_ids.view(-1)
        # 将1D的region_ids转换为嵌入向量
        x = self.embedding(region_ids)  # [batch_size, 2]
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
        return gpytorch.distributions.MultivariateNormal(mean_x, covar_x)

class GPEnhancedRegressor(nn.Module):
    """融合高斯过程的回归器"""
    def __init__(self, lstm_input_size, num_regions):
        super().__init__()
        # 原回归器结构
        self.base_regressor = EnhancedRegressor(lstm_input_size)
        
        # 高斯过程组件
        self.gp = SpatialGP(num_regions)
        self.gp_likelihood = gpytorch.likelihoods.GaussianLikelihood()
        
        # 混合权重（初始偏向深度学习部分）
        self.alpha = nn.Parameter(torch.tensor([0.7]))

    def forward(self, x, region_ids, return_uncertainty=False):
        # 基础预测
        base_pred = self.base_regressor(x)
        
        # 高斯过程预测
        with gpytorch.settings.num_likelihood_samples(32):
            gp_output = self.gp(region_ids)
            gp_mean = gp_output.mean.unsqueeze(-1)
        
        # 混合预测结果
        combined = self.alpha * base_pred + (1 - self.alpha) * gp_mean
        
        if return_uncertainty:
            # 计算预测不确定性
            gp_variance = gp_output.variance.unsqueeze(-1)
            uncertainty = (1 - self.alpha) * gp_variance
            return combined, uncertainty
        
        return combined

    def configure_gp_loss(self):
        """返回GP专用的损失函数"""
...
class CropNoiseAugment:
    ...
    def __call__(self, x):
        ...
        if self.simmtm_mask and np.random.rand() < 0.9:
            ...
            # plt.subplot(211)
            # plt.imshow(x[:, :, 0].T)  # 显示第一个波段
            # plt.title("原始时序数据")
            # plt.subplot(212)
            # plt.imshow(y[:, :, 0].T)
            # plt.title("SimMTM掩码后数据")
            # plt.tight_layout()
            # plt.show()
            x = y
...
# 修改后的AugmentedDataset类
class AugmentedDataset(TensorDataset):
    def __init__(self, x_tensor, y_tensor, region_ids, augment=None):
        super().__init__(x_tensor, y_tensor)
        self.region_ids = region_ids  # 存储地区ID
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            x = torch.FloatTensor(x_aug).to(x.device)  # 保留原设备
...
class Conv3DxLSTMModel(nn.Module):
    def __init__(self, num_regions, input_shape=(16, 16, 32)):
        super().__init__()
        self.cnn = nn.Sequential(
            ...
            nn.BatchNorm3d(128),
            nn.SiLU(),
            SpatioTemporalAttention3D(128),  # p
        ...
        # 修改回归器部分
        self.regressor = GPEnhancedRegressor(
    ...
    def forward(self, x, region_ids):  # 添加region_ids参数
        ...
        # 添加地区ID输入
        return self.regressor(lstm_out, region_ids)  # 传入region_ids
...
def train_model(model, train_loader, val_loader, epochs=100, device='cpu'):
    # 分离优化器设置
    dl_optimizer = optim.AdamW(
        list(model.cnn.parameters()) + 
        list(model.xlstm.parameters()) +
        list(model.regressor.base_regressor.parameters()),
        lr=1e-4, 
        weight_decay=1e-5,
        betas=(0.9, 0.999)
    )
    
    gp_optimizer = optim.AdamW(
        list(model.regressor.gp.parameters()) +
        list(model.regressor.gp_likelihood.parameters()) +
        [model.regressor.alpha],
        lr=1e-3,
        weight_decay=1e-4
    )
    
    # 双损失函数
    dl_criterion = nn.L1Loss()
    gp_mll = model.regressor.configure_gp_loss()
    
    # 双调度器
    dl_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(dl_optimizer, T_0=20)
    gp_scheduler = optim.lr_scheduler.LambdaLR(gp_optimizer, lr_lambda=lambda epoch: 0.9**epoch)

    best_loss = float('inf')
    history = {'train_loss': [], 'val_loss': [], 'gp_loss': []}
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, unit="batch", 
            ...
            for X, y, region_ids in pbar:  # 正确解包三个元素
                X = X.to(device)  # Tensor形状: [B, T, Band, Bin]
                y = y.to(device).float()
                region_ids = region_ids.to(device)

                # ==== 深度学习部分训练 ====
                dl_optimizer.zero_grad()
                pred = model(X, region_ids)
                loss_dl = dl_criterion(pred, y)
                ...
                gp_output = model.regressor.gp(region_ids)
                loss_gp = -gp_mll(gp_output, y)  # 注意负号（ELBO最大化）
                loss_gp.backward()
                gp_optimizer.step()

                # 记录损失
                batch_loss = loss_dl.item() + loss_gp.item()
                train_loss += batch_loss
                gp_total_loss += loss_gp.item()
                
                pbar.set_postfix({
                    'Total Loss': f"{batch_loss:.4f}",
                    'GP Loss': f"{loss_gp.item():.4f}",
                    'DL LR': f"{dl_optimizer.param_groups[0]['lr']:.2e}",
                    'GP LR': f"{gp_optimizer.param_groups[0]['lr']:.2e}"
                })
        ...
        val_loss = 0
        with torch.no_grad():
            for X, y, region_ids in val_loader:
                X, y = X.to(device), y.to(device)
                pred = model(X, region_ids)
                val_loss += dl_criterion(pred, y).item()

        avg_train = train_loss / len(train_loader)
        avg_gp = gp_total_loss / len(train_loader)
        ...
        print(f"Epoch {epoch+1}: "
              f"Train {avg_train:.4f} (GP {avg_gp:.4f}) | "
              f"Val {avg_val:.4f}")
    
    # 绘制损失曲线（新增GP损失）
    plt.figure(figsize=(12,6))
    plt.plot(history['train_loss'], label='Total Training Loss')
    plt.plot(history['gp_loss'], '--', label='GP Component')
    #plt.plot(history['val_loss'], label='Validation Loss')
    plt.title('Training Progress with Spatial GP')
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    df_meta['region_id'] = region_encoder.fit_transform(
        df_meta['province_code'] + '_' + df_meta['city_code']
    )
    ...
    train_dataset = AugmentedDataset(
        ...
        region_ids=torch.LongTensor(df_meta[~test_mask]['region_id'].values),
        augment=CropNoiseAugment()
    )
    ...
    # 新增地区编码数量获取
    num_regions = len(df_meta['region_id'].unique())
    
    model = Conv3DxLSTMModel(
        input_shape=(TIME_STEPS, BANDS, BINS),
        num_regions=num_regions  # 必须传递该参数
    ).to(device)
    ...
    # 结果保存（保持原有格式）
    with pd.ExcelWriter(f"{TEST_YEAR}年产量预测评估_3DCNN_xLSTM_GP.xlsx") as writer:
